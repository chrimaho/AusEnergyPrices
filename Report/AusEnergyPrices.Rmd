...
```{r AEMO: Cover Image, echo=FALSE, eval=TRUE, results="hide", warning=FALSE, message=FALSE, fig.width=8, fig.height=3.6}
...
aemo_MonthlyTS_avg_price[,"NSW"] %>% 
    ...
    (function(x){
        ...
        plot <- x %>% 
            ...
            geom_label(
                data=data.frame(
                    label = c("Historic NSW Price","Future NSW Price")
                    ,x=c(2013,2017.9)
                    ,y=c(60,66)
                    )
...
*`r figure(name="LongTermForecastPlotByState", caption="Long Term Forecast for each State")`*
